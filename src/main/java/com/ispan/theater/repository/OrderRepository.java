...
public interface OrderRepository extends JpaRepository<Order, Integer> {
	...
	//distinct
	@Query(value="select ROW_NUMBER() over(order by Seat.seat_id asc) as 'no',od.order_id,c.location_category+c.name as 'location',m.name as movie_name,substring(CONVERT(varchar,s.Start_time),1,19) as Start_time,CONVERT(varchar, a.auditorium_number)+'廳'+CONVERT(varchar, Seat.seat_row)+'排'+CONVERT(varchar, seat_column)+'位' as seat,substring(CONVERT(varchar,t.create_date),1,19) as create_date from OrderDetail as od join Ticket as t on od.ticket_id=t.Ticket_id join Screening as s on t.Screening_id=s.Screening_id join movie as m on m.movie_id=t.movie_id  join Seat on Seat.seat_id=t.seat_id join auditorium as a on a.auditorium_id=s.auditorium_id join cinema as c on a.cinema_id=c.cinema_id where od.order_id= 11",nativeQuery=true)
